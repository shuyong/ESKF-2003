...
static inline Eigen::QuaternionJPL<Scalar> operator + (const Eigen::QuaternionJPL<Scalar>& a,
                                      const Eigen::QuaternionJPL<Scalar>& b) {
  return Eigen::QuaternionJPL<Scalar>(a.w() + b.w(),
                                      a.x() + b.x(),
                                      a.y() + b.y(),
                                      a.z() + b.z());
}
...
template <typename Scalar>
static inline Eigen::QuaternionJPL<Scalar> operator * (const Eigen::QuaternionJPL<Scalar>& q,
                                      Scalar s) {
  return Eigen::QuaternionJPL<Scalar>(q.w() * s,
                                      q.x() * s,
                                      q.y() * s,
                                      q.z() * s);
}
...
template <typename Scalar>
static inline Eigen::QuaternionJPL<Scalar> operator * (Scalar s, const Eigen::QuaternionJPL<Scalar>& q) {
  return Eigen::QuaternionJPL<Scalar>(q.w() * s,
                                      q.x() * s,
                                      q.y() * s,
                                      q.z() * s);
 ...
 */
template <typename Scalar>
static inline void integrateEuler(Eigen::QuaternionJPL<Scalar>& q, const Eigen::QuaternionJPL<Scalar>& w, Scalar dt) {
  // eq.(23)
  q = q + static_cast<Scalar>(0.5) * (w * dt * q);
 ...
 */
template <typename Scalar>
static inline void integrateRungeKutta4(Eigen::QuaternionJPL<Scalar>& q, const Eigen::QuaternionJPL<Scalar>& w, Scalar dt) {
  ...
  Eigen::QuaternionJPL<Scalar> qw = half * w *  q;
  Eigen::QuaternionJPL<Scalar> k2 = half * w * (q + qw * dt * half);
  Eigen::QuaternionJPL<Scalar> k3 = half * w * (q + k2 * dt * half);
  Eigen::QuaternionJPL<Scalar> k4 = half * w * (q + k3 * dt);
...
void AttitudeESKF::reset() {
  ...
  dq_ = quat(a_[0] / 2.0, a_[1] / 2.0, a_[2] / 2.0, w);
  // eq.(13) q = δq(a) ⊗ q_ref
  q_ref_ = dq_ * q_ref_;
  // eq.(22)
  q_ref_.normalize();
...
void AttitudeESKF::measurementUpdateQuat(const vec3& ma, scalar_t dt) {
  ...
  auto w = 1.0 - norm_sqr / 8.0;
  dq_ = quat(a_obs_[0] / 2.0, a_obs_[1] / 2.0, a_obs_[2] / 2.0, w);
